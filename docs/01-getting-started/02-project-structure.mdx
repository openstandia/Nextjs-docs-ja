...
このページでは、Next.js アプリケーションのプロジェクト構造の概要を提供します。トップレベルのファイルとフォルダ、設定ファイル、および `app` と `pages` ディレクトリ内のルーティング規約をカバーしています。
...
| [`instrumentation.ts`](/docs/app/building-your-application/optimizing/instrumentation)      | OpenTelemetry とインスツルメンテーションファイル |
