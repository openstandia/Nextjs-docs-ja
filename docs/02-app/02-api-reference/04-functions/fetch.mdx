...
ブラウザにおいて、`cache` オプションはフェッチリクエストが*ブラウザの\_HTTPキャッシュとどのように相互作用するかを示します；この拡張により、`cache` は*サーバーサイド\_のフェッチリクエストがフレームワークの永続的な[データキャッシュ](/docs/app/building-your-application/caching#data-cache)とどのように相互作用するかを示します；
