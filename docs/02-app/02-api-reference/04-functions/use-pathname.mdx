...
> - 互換モード：
>   - [fallback route](https://nextjs.org/docs/canary/pages/api-reference/functions/get-static-paths#fallback-true) がレンダリングされている場合や、Next.js によって自動的に[静的に最適化](https://nextjs.org/docs/canary/pages/building-your-application/rendering/automatic-static-optimization)された `pages` ディレクトリのページがレンダリングされている場合、`usePathname` は `null` を返すことがあります。
