...
全パッケージを自動的にバンドル化するには（app routerのデフォルト設定）、`next.config.js`で[`bundlePagesRouterDependencies`](https://nextjs.org/docs/canary/pages/api-reference/next-config-js/bundlePagesRouterDependencies)オプションを使用します。
...
[`bundlePagesRouterDependencies`](https://nextjs.org/docs/canary/pages/api-reference/next-config-js/bundlePagesRouterDependencies)オプションを有効にした場合、`next.config.js`で[`serverExternalPackages`](https://nextjs.org/docs/canary/pages/api-reference/next-config-js/serverExternalPackages)オプションを使用して、特定のパッケージを自動的なバンドル化から除外できます。
