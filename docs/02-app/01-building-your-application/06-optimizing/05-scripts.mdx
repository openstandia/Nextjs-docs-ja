...
追加の設定を追加したい場合は、[カスタム `_document.js`](https://nextjs.org/docs/canary/pages/building-your-application/routing/custom-document) で使用される `<Head />` コンポーネント内に含めることができます：
...
> **注意**： [アセットプレフィックス](https://nextjs.org/docs/canary/pages/api-reference/next-config-js/assetPrefix) を使用しており、Partytown のデフォルト設定を変更したい場合は、それを `lib` パスの一部として含める必要があります。
...
各イベントハンドラについて詳しく学び、例を見るためには [`next/script`](https://nextjs.org/docs/canary/pages/api-reference/components/script#onload) API リファレンスをご参照ください。
