...
`loading.js`に加えて、独自のUIコンポーネントのためのSuspense境界を手動で作成することもできます。アプリルーターは、[Node.jsおよびEdgeランタイム](/docs/app/building-your-application/rendering/edge-and-nodejs-runtimes)向けに[Suspense](https://react.dev/reference/react/Suspense)を使用したストリーミングをサポートしています。
