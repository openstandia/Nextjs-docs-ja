...
`next/headers`の[`cookies`](/docs/app/api-reference/functions/cookies)を使用してクッキーを読み取ったり設定したりできます。このサーバー関数は、Route Handlerで直接呼び出したり、別の関数内にネストしたりすることができます。
...
基礎となるWeb APIを使用して、リクエストからクッキーを読み取ることもできます（[`NextRequest`](/docs/app/api-reference/functions/next-request)）：
...
> 続行する前に、[定義ルート](/docs/app/building-your-application/routing/defining-routes)ページを読むことをおすすめします。
...
Route Handlersは、ページやレイアウトと同じ[route segment設定](/docs/app/api-reference/file-conventions/route-segment-config)を使用します。
