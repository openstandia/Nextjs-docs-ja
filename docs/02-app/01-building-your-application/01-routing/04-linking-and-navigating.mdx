...
`next/link`からインポートし、このコンポーネントに`href`プロップを渡して使用できます：
...
`<Link>`に渡すことができるその他のオプションプロップもあります。詳細は[APIリファレンス](/docs/app/api-reference/components/link)を参照してください。
