...
### Route Handlers でのデータフェッチ {#fetching-data-with-route-handlers}
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  // セッションが存在しない場合、401 を返し unauthorized.tsx をレンダリングする
  if (!session) {
    unauthorized()
  }
...
export async function GET() {
  ...
  // ユーザーが認証されていない場合、401 を返し unauthorized.tsx をレンダリングする
  if (!session) {
    unauthorized()
  }
