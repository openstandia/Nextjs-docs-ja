...
`unauthorized`関数は、Next.js 401エラーページを表示するエラーをスローします。アプリケーションで認証エラーを処理するのに便利です。UIは[`unauthorized.js`ファイル](/docs/app/api-reference/file-conventions/unauthorized)を使ってカスタマイズできます。
...
`unauthorized`関数を使用して、ログインUIを含む`unauthorized.js`ファイルを表示できます。
...
特定のミューテーションを認証されたユーザーのみが行えるようにするため、Server Actionsで`unauthorized`を呼び出すことができます。
