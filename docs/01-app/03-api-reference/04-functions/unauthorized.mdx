...
`unauthorized`関数は、Next.jsの401エラーページをレンダリングするエラーをスローします。これはアプリケーション内での認可エラーの処理に便利です。UIは[`unauthorized.js`ファイル](/docs/app/api-reference/file-conventions/unauthorized)でカスタマイズできます。

`unauthorized`を使用開始するには、`next.config.js`ファイルで実験的な[`authInterrupts`](/docs/app/api-reference/config/next-config-js/authInterrupts)構成オプションを有効にしてください：
...
### 非認証ユーザーにログインUIを表示する {#displaying-login-ui-to-unauthenticated-users}
...
Server Actions内で`unauthorized`を呼び出して、特定のミューテーションを実行できるのは認証されたユーザーのみであることを確保できます。
...
Route Handlersで`unauthorized`を利用して、認証されたユーザーのみがエンドポイントにアクセスできるようにできます。
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  // セッションが存在しない場合、401を返してunauthorized.tsxをレンダリングします
  if (!session) {
    unauthorized()
  }
...
export async function GET() {
  ...
  // ユーザーが認証されていない場合、401を返してunauthorized.tsxをレンダリングします
  if (!session) {
    unauthorized()
  }
