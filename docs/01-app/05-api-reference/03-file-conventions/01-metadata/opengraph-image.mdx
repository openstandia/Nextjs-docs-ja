...
`opengraph-image.(jpg|jpeg|png|gif)` 画像と同じルートセグメントに、対応する `opengraph-image.alt.txt` ファイルを追加します。これはaltテキストです。
...
`twitter-image.(jpg|jpeg|png|gif)` 画像と同じルートセグメントに、対応する `twitter-image.alt.txt` ファイルを追加します。これはaltテキストです。
...
> - 同じファイル内で複数の画像を生成するには、[`generateImageMetadata`](/docs/app/api-reference/functions/generate-image-metadata)を使用できます。
