---
title: 'Script'
description: 'Next.jsアプリケーションで組み込みの`next/script`コンポーネントを使用してサードパーティスクリプトを最適化します。'
---
...
このAPIリファレンスは、Scriptコンポーネントで利用可能な[props](#props)の使い方を理解するのに役立ちます。機能と使用法については、[スクリプトの最適化](/docs/app/guides/scripts)ページをご覧ください。
...
`afterInteractive`戦略を使用するスクリプトは、HTMLクライアントサイドに注入され、ページの一部（またはすべて）がハイドレーションされた後に読み込まれます。**これはScriptコンポーネントのデフォルト戦略です**。最初のパーティNext.jsコードの前に読み込む必要がないスクリプトに使用するべきです。
