...
> この機能は`next export`にのみ限定されており、現在は`pages`での`getStaticPaths`や`app`での`generateStaticParams`が推奨されています。
...
返されるオブジェクトはページのマップで、`key`が`pathname`であり、`value`は以下のフィールドを受け入れるオブジェクトです：
