...
以下のページを持つアプリのためにカスタム`exportPathMap`を作成する例から始めましょう：
...
> エクスポートされた`pathname`はファイル名（例えば、`/readme.md`）にすることもできますが、その内容を提供する際に`.html`と異なる場合は`Content-Type`ヘッダーを`text/html`に設定する必要があるかもしれません。
