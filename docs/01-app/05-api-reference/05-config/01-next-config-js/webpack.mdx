...
- `dev`: `Boolean` - 開発環境でのコンパイルかどうかを示します
...
`isServer` が `true` であるとき、`nextRuntime` は `"edge"` または `"nodejs"` です。`nextRuntime` の `"edge"` は現在、edge runtime のミドルウェアと Server Components のみを対象としています。
