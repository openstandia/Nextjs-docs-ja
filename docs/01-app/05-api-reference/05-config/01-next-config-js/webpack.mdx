...
- `nextRuntime`: `String | undefined` - サーバーサイドのコンパイルのターゲットランタイム；`"edge"` または `"nodejs"` のいずれかで、クライアントサイドのコンパイルの場合は `undefined`
...
`isServer` が `true` であるとき、`nextRuntime` が `"edge"` または `"nodejs"` であることに注意してください。`nextRuntime` `"edge"` は現在、edge runtime でのミドルウェアと Server Components のみを対象としています。
