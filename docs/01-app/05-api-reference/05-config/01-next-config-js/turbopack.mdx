...
`turbopack`設定で利用可能なオプションは以下の通りです：
...
Next.jsはプロジェクトのrootディレクトリを自動的に検出します。以下のいずれかのファイルを探すことで行います：
...
- webpackローダーに渡されるオプションは、純粋なJavaScriptプリミティブ、オブジェクト、および配列でなければなりません。例えば、`require()`プラグインモジュールをオプション値として渡すことはできません。
...
Turbopackは、webpackの[`resolve.alias`](https://webpack.js.org/configuration/resolve/#resolvealias)設定に似た方法で、エイリアスを通じてモジュール解決を変更するように設定できます。
...
webpackからTurbopackへのアプリの移行方法についての詳細情報とガイダンスについては、[Turbopackのwebpack互換性に関するドキュメント](https://turbo.build/pack/docs/migrating-from-webpack)を参照してください。
