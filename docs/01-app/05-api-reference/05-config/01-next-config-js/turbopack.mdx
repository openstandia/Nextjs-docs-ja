...
- webpackローダーAPIのコアサブセットのみが実装されています。現在、一部の人気のあるローダーに対して十分なカバレッジがあり、将来的にはAPIサポートを拡大する予定です。
...
以下は、`.svg`ファイルをインポートしてReactコンポーネントとしてレンダリングするための[`@svgr/webpack`](https://www.npmjs.com/package/@svgr/webpack)ローダーを使用した例です。
