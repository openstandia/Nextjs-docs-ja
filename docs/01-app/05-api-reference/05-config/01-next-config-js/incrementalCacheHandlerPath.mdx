...
description: Redis、Memcachedなどの外部サービスを使用してデータを保存および再検証するために使用されるNext.jsキャッシュを設定します。
---

Next.jsアプリケーションの複数のコンテナやインスタンス間でキャッシュを共有したり、キャッシュされたページやデータを永続的なストレージに保存したい場合、Next.jsのキャッシュの場所を設定することができます。
...
Next.jsをセルフホスティングする際の[ISRの設定](/docs/app/guides/self-hosting#caching-and-isr)方法を学んでください。
