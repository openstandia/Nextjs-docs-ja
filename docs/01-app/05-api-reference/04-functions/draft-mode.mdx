...
その後、Route Handler を呼び出すリクエストを送信します。[`<Link>` コンポーネント](/docs/app/api-reference/components/link)を使用してルートを呼び出す場合、prefetch 時に誤って cookie を削除しないようにするために `prefetch={false}` を渡す必要があります。
