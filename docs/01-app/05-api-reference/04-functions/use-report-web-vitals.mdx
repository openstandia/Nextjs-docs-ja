...
> `useReportWebVitals` フックは `"use client"` ディレクティブを必要とするため、最もパフォーマンスの良いアプローチは、root レイアウトがインポートする別のコンポーネントを作成することです。これにより、client boundary は `WebVitals` コンポーネントに限定されます。
...
> ```js
> useReportWebVitals(metric => {
>   // Google Analytics を次の例のように初期化した場合は `window.gtag` を使用します：
...
> ```
>
> [Google Analytics に結果を送信する](https://github.com/GoogleChrome/web-vitals#send-the-results-to-google-analytics) について詳しく読む。
