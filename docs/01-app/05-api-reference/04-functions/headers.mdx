...
- `headers`は、事前に返される値を知ることができない[Dynamic API](/docs/app/building-your-application/rendering/server-components#server-rendering-strategies#dynamic-apis)です。これを使用すると、ルートが**[動的レンダリング](/docs/app/building-your-application/rendering/server-components#dynamic-rendering)**に切り替わります。
