...
- `props` - 現在のルートのパラメータを含むオブジェクト：
  ...
  - `params` - [動的ルートパラメータ](/docs/app/building-your-application/routing/dynamic-routes)オブジェクトを含むオブジェクト。`generateMetadata`が呼び出されるセグメントまでのrootセグメントからのオブジェクト。例：
...
> **Deprecated**: `metadata`の`themeColor`オプションはNext.js 14で非推奨となりました。代わりに[`viewport`設定](/docs/app/api-reference/functions/generate-viewport)を使用してください。

### `colorScheme` {#colorscheme}

> **Deprecated**: `metadata`の`colorScheme`オプションはNext.js 14で非推奨となりました。代わりに[`viewport`設定](/docs/app/api-reference/functions/generate-viewport)を使用してください。
...
> **Deprecated**: `metadata`の`viewport`オプションはNext.js 14で非推奨となりました。代わりに[`viewport`設定](/docs/app/api-reference/functions/generate-viewport)を使用してください。
...
記録、文書、またはその他の歴史的関心のある資料のコレクションを説明します（[ソース](https://www.w3.org/TR/2011/WD-html5-20110113/links.html#rel-archives)）。
...
ページメタデータは主にボットやクローラーを対象としているため、Next.jsはJavaScriptを実行し、完全なページDOMを検査できるボット（例：`Googlebot`）にメタデータをストリーミングします。ただし、メタデータは**HTML限定**のボット（例：`Twitterbot`）に対してはページのレンダリングをブロックし続けます。これらのボットはクロール中にJavaScriptを実行できません。
