...
このページでは、`generateMetadata`と静的メタデータオブジェクトを使用した**Config-based Metadata**オプションについて説明します。
...
> **Good to know**:
>
> - メタデータが実行時情報に依存しない場合は、`generateMetadata`ではなく静的な[`metadata`オブジェクト](#the-metadata-object)を使用して定義するべきです。
...
> **Good to know**:
>
> - `metadataBase`は、すべてのルートにわたるURLベースの`metadata`フィールドに適用するために、root `app/layout.js`で設定されることが一般的です。
...
>
> これらの値が存在する場合、それらは`metadataBase`の**デフォルト値**として使用されます。存在しない場合は、`http://localhost:${process.env.PORT || 3000}`にフォールバックします。これにより、Open Graph画像がローカルビルドとVercelプレビューおよび本番デプロイメントの両方で機能します。デフォルトを上書きする場合は、環境変数を使用してURLを計算することをお勧めします。これにより、ローカル開発、ステージング、および本番環境のURLを設定できます。
>
...
`<link>`要素には、ブラウザが外部リソースを必要とする可能性があることを示すために使用できる`rel`キーワードがいくつかあります。ブラウザはこの情報を使用して、キーワードに応じてプリロード最適化を適用します。
