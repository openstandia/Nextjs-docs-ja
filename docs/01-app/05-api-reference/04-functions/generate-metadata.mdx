...
- `props` - 現在のルートのパラメータを含むオブジェクト：
  ...
  - `params` - [動的ルートパラメータ](/docs/app/building-your-application/routing/dynamic-routes)オブジェクトを含むオブジェクトで、rootセグメントから`generateMetadata`が呼び出されるセグメントまで。例：
...
`metadataBase`は、完全修飾URLを必要とする`metadata`フィールドに対してベースURLプレフィックスを設定するための便利なオプションです。
...
> **Good to know**:
>
> - `metadataBase`は通常、rootの`app/layout.js`で設定され、すべてのルートにわたってURLベースの`metadata`フィールドに適用されます。
> - 絶対URLを必要とするすべてのURLベースの`metadata`フィールドは、`metadataBase`オプションで構成できます。
> - `metadataBase`にはサブドメイン（例：`https://app.acme.com`）やベースパス（例：`https://acme.com/start/from/here`）を含めることができます。
...
> **Good to know**:
>
> - Open Graph画像には、[ファイルベースのメタデータAPI](/docs/app/api-reference/file-conventions/metadata/opengraph-image#image-files-jpg-png-gif)を使用する方が便利です。設定エクスポートと実際のファイルを同期する必要がなく、ファイルベースのAPIが自動的に正しいメタデータを生成します。
...
> **Good to know**: 可能であれば、アイコンには[ファイルベースのメタデータAPI](/docs/app/api-reference/file-conventions/metadata/app-icons#image-files-ico-jpg-png)を使用することをお勧めします。設定エクスポートと実際のファイルを同期する必要がなく、ファイルベースのAPIが自動的に正しいメタデータを生成します。
...
記録、文書、またはその他の歴史的に興味深い資料のコレクションを説明します（[ソース](https://www.w3.org/TR/2011/WD-html5-20110113/links.html#rel-archives)）。
...
特定のFacebookソーシャルプラグインに対して、FacebookアプリまたはFacebookアカウントをWebページに接続できます。[Facebookドキュメント](https://developers.facebook.com/docs/plugins/comments/#moderation-setup-instructions)
...
| `<link rel="stylesheet" />`   | レイアウトまたはページ自体でスタイルシートを`import`します。                                                                                                                                                                                                         |
...
`<link>`要素には、ブラウザが外部リソースが必要になる可能性が高いことを示唆するために使用できる`rel`キーワードがいくつかあります。ブラウザはこの情報を使用して、キーワードに応じてプリロード最適化を適用します。
...
> **Good to know**:
>
> - これらのメソッドは現在、クライアントコンポーネントでのみサポートされていますが、初期ページロード時にはサーバーサイドレンダリングされます。
> - Next.jsの組み込み機能である`next/font`、`next/image`、`next/script`は、関連するリソースヒントを自動的に処理します。

## タイプ {#types}

`Metadata`タイプを使用してメタデータに型安全性を追加できます。IDEで[組み込みのTypeScriptプラグイン](/docs/app/api-reference/config/typescript)を使用している場合、手動でタイプを追加する必要はありませんが、必要に応じて明示的に追加することもできます。
...
ページメタデータは主にボットやクローラーを対象としているため、Next.jsはJavaScriptを実行し、完全なページDOMを検査できるボット（例：`Googlebot`）に対してメタデータをストリーミングします。ただし、メタデータは**HTML限定**ボット（例：`Twitterbot`）に対してはページのレンダリングをブロックし続けます。これらのボットはクロール中にJavaScriptを実行できないためです。
