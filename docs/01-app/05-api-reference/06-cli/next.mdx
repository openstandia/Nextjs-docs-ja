...
これらの値はどちらも[**gzipで圧縮**](/docs/app/api-reference/config/next-config-js/compress)されています。最初のロードは緑、黄、または赤で示されます。パフォーマンスの良いアプリケーションを目指すために緑を目指しましょう。
...
| --------------------------------------- | ------------------------------------------------------------------------------------------------ |
| `-h` または `--help`                    | 利用可能なすべてのオプションを表示します。                                                       |
| `[directory]`                           | アプリケーションを起動するディレクトリ。指定されていない場合、現在のディレクトリが使用されます。 |
| `-p` または `--port <port>`             | アプリケーションを起動するポート番号を指定します。（デフォルト：3000、環境変数：PORT）           |
...
> **Good to know**: `PORT`は、HTTPサーバーの起動が他のコードが初期化される前に行われるため、`.env`で設定することはできません。
