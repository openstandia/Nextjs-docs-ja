...
| **React Server Components (RSC)** | **サポート** | Next.js App Router用。Turbopackは正しいサーバー/クライアントのバンドリングを保証します。               |
...
| **PostCSS**         | **サポート**           | `postcss.config.js`をNode.jsワーカープールで自動的に処理します。Tailwind、Autoprefixerなどに便利です。                                                                                                        |
...
| **手動エイリアス** | **サポート**     | [`next.config.js`で`resolveAlias`を設定](/docs/app/api-reference/config/next-config-js/turbopack#resolving-aliases)（`webpack.resolve.alias`に類似）。 |
| **カスタム拡張子** | **サポート**     | [`next.config.js`で`resolveExtensions`を設定](/docs/app/api-reference/config/next-config-js/turbopack#resolving-custom-extensions)。                   |
