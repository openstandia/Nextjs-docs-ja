...
- **バンドリング vs ネイティブESM:** 一部のツールは開発中にバンドリングをスキップし、ブラウザのネイティブESMに依存します。これは小規模なアプリには適していますが、ネットワークリクエストが多すぎるため、大規模なアプリでは遅くなる可能性があります。Turbopackは開発中に**バンドル**しますが、大規模なアプリを高速に保つために最適化されています。
...
- **`resolveAlias`**
  手動エイリアスを作成します（webpackの`resolve.alias`のように）。
- **`resolveExtensions`**
...
Turbopackの改善とプロダクションビルドサポートの追加に向けて、今後の更新にご期待ください。その間、`next dev --turbopack`を試してみて、フィードバックをお寄せください。
