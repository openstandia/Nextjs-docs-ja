...
<Image
  alt="ルートセグメントにpage.jsまたはroute.jsファイルが含まれていても、コロケーションされたプロジェクトファイルはルーティング可能ではないことを示す図。"
...
> **Good to know**: プロジェクトファイルを`app`にコロケーションすることは**可能**ですが、必ずしもそうする必要はありません。必要に応じて、[それらを`app`ディレクトリの外に保管する](#store-project-files-outside-of-app)こともできます。
