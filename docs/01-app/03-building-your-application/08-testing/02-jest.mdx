...
JestとReact Testing Libraryは、**Unit Testing**と**Snapshot Testing**のためによく一緒に使用されます。このガイドでは、JestをNext.jsで設定し、最初のテストを書く方法を紹介します。

> **Good to know:** `async` server componentはReactエコシステムにおいて新しいため、Jestは現在それをサポートしていません。同期的なserver componentとclient componentのための**unit tests**を実行することは可能ですが、`async` componentには**E2E tests**を使用することを推奨します。
...
`jest --watch`は、ファイルが変更されるとテストを再実行します。その他のJest CLIオプションについては、[Jest Docs](https://jestjs.io/docs/cli#reference)を参照してください。
