...
[Next.js 12](https://nextjs.org/blog/next-12)のリリース以降、Next.jsにはJestのための組み込みの設定が追加されました。
...
// createJestConfigはこの方法でエクスポートされ、next/jestが非同期のNext.js設定を読み込むことができるようにします
...
内部では、`next/jest`が自動的にJestを設定しており、以下を含みます：
...
`@testing-library/jest-dom`には、`.toBeInTheDocument()`などの便利な[カスタムマッチャー](https://github.com/testing-library/jest-dom#custom-matchers)が含まれており、テストを書くのが容易になります。Jest設定ファイルに次のオプションを追加することで、すべてのテストにカスタムマッチャーをインポートできます：
