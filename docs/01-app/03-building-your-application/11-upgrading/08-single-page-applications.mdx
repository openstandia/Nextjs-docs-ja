---
title: 'Next.jsでのシングルページアプリケーション'
...
既存のSPAがある場合、大きなコード変更をせずにNext.jsに移行できます。その後、必要に応じてサーバー機能を段階的に追加することができます。
...
[`next/link`](/docs/app/api-reference/components/link)コンポーネントはルートを自動的に[プリフェッチ](/docs/app/api-reference/components/link#prefetch)し、厳密なSPAの高速なページ遷移を提供しますが、アプリケーションのルーティング状態をURLに永続化してリンクや共有に利用する利点があります。
...
データ取得をroot レイアウトに「持ち上げる」ことで、Next.jsはアプリケーション内の他のコンポーネントよりも早くサーバーで指定されたリクエストを開始します。これによりクライアントのウォーターフォールが排除され、クライアントとサーバー間の複数の往復を防ぐことができます。また、サーバーがデータベースの近く（理想的には同じ場所）にあるため、パフォーマンスが大幅に向上する可能性があります。
...
初期の`fallback`データはNext.jsによって自動的に処理されるため、`data`が`undefined`であるかどうかを確認するために以前必要だった条件付きロジックを削除できます。データが読み込まれているときは、最も近い`<Suspense>`境界がサスペンドされます。
...
Next.jsとともにReact Queryをクライアントとサーバーの両方で使用できます。これにより、厳密なSPAを構築することができ、Next.jsのサーバー機能とReact Queryを組み合わせて利用することができます。
...
[Create React App](/docs/app/building-your-application/upgrading/from-create-react-app)や[Vite](/docs/app/building-your-application/upgrading/from-vite)のような厳密なSPAから移行する場合、URL状態を更新するためにシャロールートする既存のコードがあるかもしれません。これは、Next.jsのデフォルトのファイルシステムルーティングを使用せずに、アプリケーション内のビュー間を手動で遷移するのに役立ちます。
...
クライアントコンポーネントを使用しながらServer Actionsを段階的に採用できます。これにより、APIルートを呼び出すためのボイラープレートコードを削除し、代わりに`useActionState`のようなReact機能を使用してローディングやエラーステートを処理できます。
...
クライアントからServer Actionをインポートして使用できます。JavaScript関数を呼び出すのと同様です。APIエンドポイントを手動で作成する必要はありません：
...
Pages Routerを使用しているSPAを既に使用している場合は、[App Routerを段階的に採用する方法](/docs/app/building-your-application/upgrading/app-router-migration)を学ぶことができます。
