...
レイアウトやページ内のすべての `fetch` リクエストをキャッシュに含めるには、`export const fetchCache = 'default-cache'` [segment config オプション](/docs/app/api-reference/file-conventions/route-segment-config) を使用できます。個々の `fetch` リクエストが `cache` オプションを指定している場合は、それが使用されます。
...
[ルートハンドラー](/docs/app/api-reference/file-conventions/route) の `GET` 関数はデフォルトでキャッシュされなくなりました。`GET` メソッドをキャッシュに含めるには、`export const dynamic = 'force-static'` などの [ルート設定オプション](/docs/app/api-reference/file-conventions/route-segment-config) をルートハンドラーファイルに使用できます。
...
`<Link>` または `useRouter` を介してページ間を移動する際、[ページ](/docs/app/api-reference/file-conventions/page) セグメントはクライアントサイドルーターキャッシュから再利用されなくなりました。ただし、ブラウザのバックワードおよびフォワードナビゲーション中や共有レイアウトでは引き続き再利用されます。
