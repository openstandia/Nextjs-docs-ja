...
> **Good to know**:
>
> - `redirect` はデフォルトで 307（Temporary Redirect）ステータスコードを返します。Server Actionで使用される場合、303（See Other）を返し、これはPOSTリクエストの結果として成功ページにリダイレクトするためによく使用されます。
...
`permanentRedirect` は、エンティティの正規URLが変更された後、たとえばユーザーがユーザー名を変更した後にプロフィールURLを更新する場合によく使用されます：
...
すべての受信リクエストに対して大規模なデータセットを読み取ることは、遅くて高価になる可能性があります。データ検索パフォーマンスを最適化する方法は2つあります：
