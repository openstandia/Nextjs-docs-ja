---
title: 'Middleware'
description: 'リクエストが完了する前にコードを実行するためのMiddlewareの使い方を学びます。'
---
...
// この関数は、内部で`await`を使用する場合に`async`としてマークできます
...
Middlewareは**プロジェクト内のすべてのルート**に対して呼び出されます。したがって、特定のルートを正確にターゲットにするか除外するために、マッチャーを使用することが重要です。実行順序は以下のとおりです：
...
`matcher`設定は完全な正規表現をサポートしているため、否定の先読みや文字のマッチングなどが可能です。特定のパスを除くすべてをマッチングする否定の先読みの例を以下に示します：
...
> **Good to know**: `matcher`の値は定数である必要があるため、ビルド時に静的に解析できます。変数などの動的な値は無視されます。
...
1. 受信リクエストの場合、`cookies`には次のメソッドがあります：`get`、`getAll`、`set`、および`delete`。`has`を使用してcookieの存在を確認したり、`clear`ですべてのcookieを削除したりできます。
...
> **Good to know**: 大きなヘッダーを設定することは避けてください。バックエンドWebサーバーの設定によっては[431 Request Header Fields Too Large](https://developer.mozilla.org/docs/Web/HTTP/Status/431)エラーが発生する可能性があります。
...
Next.js 15.1から、`next/experimental/testing/server`パッケージには、ミドルウェアファイルの単体テストを支援するユーティリティが含まれています。ミドルウェアの単体テストは、意図したパスでのみ実行されることを確認し、カスタムルーティングロジックが本番コードに到達する前に意図どおりに機能することを保証するのに役立ちます。
...
次に、ミドルウェアファイルで、`config`オブジェクト内でランタイムを`nodejs`に設定します：
