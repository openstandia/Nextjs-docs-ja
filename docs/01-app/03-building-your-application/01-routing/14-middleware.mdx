...
> **Note**: プロジェクトごとに1つの`middleware.ts`ファイルのみがサポートされていますが、ミドルウェアのロジックをモジュール化して整理することは可能です。ミドルウェアの機能を個別の`.ts`または`.js`ファイルに分割し、メインの`middleware.ts`ファイルにインポートします。これにより、ルート固有のミドルウェアをクリーンに管理し、`middleware.ts`に集約して集中管理できます。単一のミドルウェアファイルを強制することで、設定が簡素化され、潜在的な競合が防止され、複数のミドルウェア層を回避することでパフォーマンスが最適化されます。
...
Middlewareは**プロジェクト内のすべてのルート**に対して呼び出されます。したがって、特定のルートを正確にターゲットにするか、除外するためにマッチャーを使用することが重要です。以下は実行順序です：
...
export const config = {
  matcher: [
    /*
     * 次のもので始まるリクエストパスを除くすべてのリクエストパスをマッチング：
     * - api（APIルート）
     * - _next/static（静的ファイル）
...
1. 受信リクエストの場合、`cookies`には以下のメソッドがあります：`get`、`getAll`、`set`、および`delete`。`has`を使用してcookieの存在を確認したり、`clear`で全てのcookieを削除できます。
2. 送信レスポンスの場合、`cookies`には以下のメソッドがあります：`get`、`getAll`、`set`、および`delete`。
...
`NextResponse` APIを使用してリクエストとレスポンスのヘッダーを設定できます（*リクエスト*ヘッダーの設定はNext.js v13.0.0から利用可能です）。
...
Middlewareから直接`Response`または`NextResponse`インスタンスを返すことで応答できます。（これは[Next.js v13.1.0](https://nextjs.org/blog/next-13-1#nextjs-advanced-middleware)から利用可能です）
...
`waitUntil()`メソッドはプロミスを引数として受け取り、プロミスが解決するまでMiddlewareの寿命を延ばします。これはバックグラウンドで作業を行うのに便利です。
...
`skipTrailingSlashRedirect`は、トレーリングスラッシュを追加または削除するためのNext.jsのリダイレクトを無効にします。これにより、特定のパスにはトレーリングスラッシュを維持し、他のパスには維持しないカスタム処理が可能になり、インクリメンタルな移行が容易になります。
...
`skipMiddlewareUrlNormalize`は、Next.jsでのURL正規化を無効にして、直接訪問とクライアント遷移の処理を同じにします。いくつかの高度なケースでは、このオプションを使用することで、元のURLを使用して完全な制御が可能になります。
...
Next.js 15.1から、`next/experimental/testing/server`パッケージにはミドルウェアファイルを単体テストするためのユーティリティが含まれています。ミドルウェアの単体テストは、意図したパスでのみ実行されることを確認し、カスタムルーティングロジックが本番コードに到達する前に意図通りに動作することを保証するのに役立ちます。
...
MiddlewareはデフォルトでEdge runtimeを使用します。v15.2（canary）から、Node.js runtimeを使用するための実験的なサポートがあります。有効にするには、`next.config`ファイルにフラグを追加します：
...
次に、ミドルウェアファイルで`config`オブジェクトに`nodejs`を設定します：
...
> **Note**: この機能はまだ本番環境での使用は推奨されていません。そのため、Next.jsは安定版リリースの代わりにnext@canaryリリースを使用していない限り、エラーをスローします。
...
Next.jsをセルフホスティングする際の[Middlewareの設定方法](/docs/app/guides/self-hosting#middleware)を学びましょう。
