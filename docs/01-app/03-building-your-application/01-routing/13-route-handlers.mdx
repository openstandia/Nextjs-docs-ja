...
Route Handlerに渡されるリクエストオブジェクトは`NextRequest`インスタンスであり、クエリパラメータをより簡単に処理するための[いくつかの追加の便利なメソッド](/docs/app/api-reference/functions/next-request#nexturl)が含まれています。
