...
description: 'WebのRequestとResponse APIを使用して、特定のルートに対するカスタムリクエストハンドラーを作成します。'
related:
  title: 'APIリファレンス'
...
ネイティブの[Request](https://developer.mozilla.org/docs/Web/API/Request)と[Response](https://developer.mozilla.org/docs/Web/API/Response) APIをサポートすることに加えて、Next.jsは[`NextRequest`](/docs/app/api-reference/functions/next-request)と[`NextResponse`](/docs/app/api-reference/functions/next-response)を拡張し、高度なユースケースに便利なヘルパーを提供します。

## 振る舞い {#behavior}

### キャッシング {#caching}

Route Handlersはデフォルトではキャッシュされません。ただし、`GET`メソッドに対してキャッシングを選択することができます。他のサポートされているHTTPメソッドは**キャッシュされません**。`GET`メソッドをキャッシュするには、Route Handlerファイル内で`export const dynamic = 'force-static'`のような[route config option](/docs/app/api-reference/file-conventions/route-segment-config#dynamic)を使用します。
...
また、リクエストからCookieを読み取るために、基盤となるWeb APIを使用することもできます（[`NextRequest`](/docs/app/api-reference/functions/next-request)）：
...
また、リクエストからヘッダーを読み取るために、基盤となるWeb APIを使用することもできます（[`NextRequest`](/docs/app/api-reference/functions/next-request)）：
...
Route Handlerに渡されるリクエストオブジェクトは`NextRequest`インスタンスであり、クエリパラメータをより簡単に処理するための[いくつかの追加の便利なメソッド](/docs/app/api-reference/functions/next-request#nexturl)を持っています。
...
特に、Pages Routerを使用したAPI Routesとは異なり、追加の設定を使用するために`bodyParser`を使用する必要はありません。
