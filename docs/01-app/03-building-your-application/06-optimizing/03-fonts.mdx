...
上記の例では、`Inter`はグローバルに適用され、任意の`<h1>`タグは`Roboto Mono`でスタイリングされます。
...
以下の例では、`next/font/google`から`Inter`と`Roboto_Mono`フォントを使用しています（任意のGoogle Fontまたはローカルフォントを使用できます）。`variable`オプションを使用して、これらのフォントに対して`inter`と`roboto_mono`というCSS変数名を定義します。次に、`inter.variable`と`roboto_mono.variable`を適用して、CSS変数をHTMLドキュメントに含めます。
...
最後に、CSS変数を[Tailwind CSSの設定](/docs/app/building-your-application/styling/tailwind-css#configuring-tailwind)に追加します:
