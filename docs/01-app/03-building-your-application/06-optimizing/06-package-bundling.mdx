...
これらのパッケージのインポート方法を最適化するには、`next.config.js`に[`optimizePackageImports`](/docs/app/api-reference/config/next-config-js/optimizePackageImports)オプションを追加します。このオプションは、実際に使用するモジュールのみを読み込みつつ、多くの名前付きエクスポートを持つインポート文を書く利便性を提供します。
...
Server ComponentsやRoute Handlers内でインポートされたパッケージはNext.jsによって自動的にバンドルされるため、`next.config.js`で[`serverExternalPackages`](/docs/app/api-reference/config/next-config-js/serverExternalPackages)オプションを使用して特定のパッケージをバンドルから除外できます。
