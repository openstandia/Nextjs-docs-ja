...
> - 最大バンドルサイズは`500KB`です。バンドルサイズには、JSX、CSS、フォント、画像、その他のアセットが含まれます。制限を超えた場合は、アセットのサイズを減らすか、実行時にフェッチすることを検討してください。
...
TypeScriptを使用してJSON-LDを型付けするには、[`schema-dts`](https://www.npmjs.com/package/schema-dts)のようなコミュニティパッケージを使用できます：
