...
`@next/mdx`パッケージと関連パッケージは、Next.jsを設定してMarkdownとMDXを処理できるようにするために使用されます。**ローカルファイルからデータを取得**し、`.md`または`.mdx`拡張子のページを`/pages`または`/app`ディレクトリに直接作成できます。
...
これにより、`.md`および`.mdx`ファイルをアプリケーション内でページ、ルート、またはインポートとして使用できるようになります。
