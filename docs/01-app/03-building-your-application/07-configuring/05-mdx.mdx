...
App Routerアプリでは、[metadata](/docs/app/building-your-application/optimizing/metadata)を使用することも含まれます。
...
> **Good to know**: インポート時に`.mdx`ファイル拡張子を指定してください。[モジュールパスエイリアス](/docs/app/getting-started/installation#set-up-absolute-imports-and-module-path-aliases)（例: `@/content`）を使用する必要はありませんが、インポートパスを簡略化できます。
