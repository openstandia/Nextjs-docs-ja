...
`permanentRedirect` は、エンティティの正規URLが変更された後、たとえばユーザー名を変更した後にユーザープロフィールURLを更新する場合によく使用されます：
