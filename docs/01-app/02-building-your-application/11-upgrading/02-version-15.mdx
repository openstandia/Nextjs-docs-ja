---
title: 'バージョン 15'
description: 'Next.js アプリケーションをバージョン 14 から 15 にアップグレードする方法。'
---
...
マイグレーションの負担を軽減するために、プロセスを自動化する[コーデモッドが利用可能](/docs/app/building-your-application/upgrading/codemods#150)であり、APIは一時的に同期的にアクセスできます。
...
以前は `edge` に加えて `experimental-edge` の値がサポートされていた `runtime` [セグメント設定](/docs/app/api-reference/file-conventions/route-segment-config#runtime)。両方の設定は同じものを指し、オプションを簡素化するために、`experimental-edge` が使用されている場合はエラーが発生します。これを修正するには、`runtime` 設定を `edge` に更新してください。[コーデモッド](/docs/app/building-your-application/upgrading/codemods#app-dir-runtime-config-experimental-edge) が自動的にこれを行います。
...
`@next/font` パッケージは、組み込みの [`next/font`](/docs/app/api-reference/components/font) に置き換えられました。[コーデモッドが利用可能](/docs/app/building-your-application/upgrading/codemods#built-in-next-font) であり、安全かつ自動的にインポートをリネームできます。
...
`NextRequest` の `geo` と `ip` プロパティは、これらの値がホスティングプロバイダーによって提供されるため削除されました。[コーデモッド](/docs/app/building-your-application/upgrading/codemods#150) がこのマイグレーションを自動化するために利用可能です。
