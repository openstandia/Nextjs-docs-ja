...
> **Good to know:** このcodemodが手動介入が必要な箇所を特定したが、正確な修正を決定できない場合、コードにコメントまたは型キャストを追加して、手動で更新する必要があることをユーザーに通知します。これらのコメントは**@next/codemod**で始まり、型キャストは`UnsafeUnwrapped`で始まります。
...
トップレベルページで自動的に注入される廃止された`url`プロパティを`withRouter`とそれが注入する`router`プロパティを使用するように変換します。詳しくはこちらをお読みください：[https://nextjs.org/docs/messages/url-deprecated](https://nextjs.org/docs/messages/url-deprecated)
