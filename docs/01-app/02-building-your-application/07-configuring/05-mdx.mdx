...
> **Good to know**:
>
> シリアライズ可能なオプションを持たないremarkとrehypeプラグインは、[Turbopack](/docs/architecture/turbopack)ではまだ使用できません。これは、[JavaScript関数をRustに渡すことができないため](https://github.com/vercel/next.js/issues/71819#issuecomment-2461802968)です。

## リモートMDX {#remote-mdx}

MDXファイルやコンテンツが*他の場所*にある場合、サーバー上で動的にフェッチできます。これは、別のローカルフォルダ、CMS、データベース、または他のどこかに保存されたコンテンツに便利です。この用途に人気のあるコミュニティパッケージは、[`next-mdx-remote`](https://github.com/hashicorp/next-mdx-remote#react-server-components-rsc--nextjs-app-directory-support)です。
