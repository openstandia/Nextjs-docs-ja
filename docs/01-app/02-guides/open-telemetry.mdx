...
このドキュメントでは、_Span_、_Trace_、*Exporter*などの用語を使用しています。これらはすべて[OpenTelemetry Observability Primer](https://opentelemetry.io/docs/concepts/observability-primer/)で確認できます。

Next.jsはOpenTelemetryのインストルメンテーションを標準でサポートしており、Next.js自体をすでにインストルメントしています。

<PagesOnly>
  OpenTelemetryを有効にすると、`getStaticProps`のようなすべてのコードを自動的に有用な属性を持つ_spans_でラップします。
</PagesOnly>
...
`@vercel/otel`パッケージは多くの設定オプションを提供し、一般的なユースケースに対応します。しかし、ニーズに合わない場合は、OpenTelemetryを手動で設定することもできます。
...
これを行うことは`@vercel/otel`を使用することと同等ですが、`@vercel/otel`では公開されていない機能を変更および拡張することが可能です。edge runtimeのサポートが必要な場合は、`@vercel/otel`を使用する必要があります。
...
- `next.page`
  ...
  - 特別なファイル（`page.ts`、`layout.ts`、`loading.ts`など）へのルートと考えることができます
