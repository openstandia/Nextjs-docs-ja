...
異なるゾーンのパスへのリンクは、Next.jsの[`<Link>`](https://nextjs.org/docs/canary/pages/api-reference/components/link)コンポーネントではなく、`a`タグを使用するべきです。これは、Next.jsが`<Link>`コンポーネント内の相対パスをプリフェッチし、ソフトナビゲーションしようとするためであり、ゾーン間では機能しません。
