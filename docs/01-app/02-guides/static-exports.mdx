...
nav_title: '静的エクスポート'
description: 'Next.js は、静的サイトやシングルページアプリケーション (SPA) として開始し、後でサーバーを必要とする機能をオプションで利用することができます。'
---
...
Next.js は、静的サイトやシングルページアプリケーション (SPA) として開始し、後でサーバーを必要とする機能をオプションで利用することができます。
...
Next.js はこの静的エクスポートをサポートしているため、HTML/CSS/JS の静的アセットを提供できる任意のウェブサーバーにデプロイしてホストすることができます。
...
結果として得られるコンポーネントは、初回ページロードのために静的 HTML にレンダリングされ、ルート間のクライアントナビゲーションのための静的ペイロードが生成されます。静的エクスポートを使用する際、Server Components に変更は必要ありませんが、[dynamic server functions](#unsupported-features) を使用する場合を除きます。
...
Route Handlers は、`next build` を実行すると静的なレスポンスをレンダリングします。サポートされているのは `GET` HTTP メソッドのみです。これを使用して、キャッシュされたデータまたはキャッシュされていないデータから静的な HTML、JSON、TXT、その他のファイルを生成できます。たとえば：
...
### ブラウザ API {#browser-apis}

Client Components は `next build` 中に HTML にプリレンダリングされます。[Web API](https://developer.mozilla.org/docs/Web/API) である `window`、`localStorage`、`navigator` などはサーバー上では利用できないため、これらの API にアクセスする際はブラウザで実行されているときにのみ安全にアクセスする必要があります。たとえば：
...
静的エクスポートを使用すると、Next.js は HTML/CSS/JS の静的アセットを提供できる任意のウェブサーバーにデプロイしてホストすることができます。
...
Nginx のような静的ホストを使用している場合、受信リクエストから正しいファイルへのリライトを設定できます：
...
| `v13.4.0`  | App Router (Stable) は、React Server Components や Route Handlers を含む強化された静的エクスポートサポートを追加しました。 |
