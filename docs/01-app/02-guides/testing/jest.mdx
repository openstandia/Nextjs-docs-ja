...
JestとReact Testing Libraryは、**ユニットテスト**や**スナップショットテスト**によく使用されます。このガイドでは、Next.jsでJestを設定し、最初のテストを書く方法を紹介します。
...
`next/jest`は内部でJestを自動的に設定しており、以下を含みます：
...
> **Good to know**: 環境変数を直接テストするには、別のセットアップスクリプトまたは`jest.config.ts`ファイルで手動でロードしてください。詳細については、[テスト環境変数](/docs/app/guides/environment-variables#test-environment-variables)をご覧ください。
...
各設定オプションの詳細については、[Jestドキュメント](https://jestjs.io/docs/configuration)をご覧ください。また、Next.jsがJestをどのように設定しているかを確認するために、[`next/jest`の設定](https://github.com/vercel/next.js/blob/e02fe314dcd0ae614c65b505c6daafbdeebb920e/packages/next/src/build/jest/jest.ts)を確認することをお勧めします。
...
静的アセットの処理に関する詳細は、[Jestドキュメント](https://jestjs.io/docs/webpack#handling-static-assets)をご覧ください。
...
`jest --watch`は、ファイルが変更されるとテストを再実行します。その他のJest CLIオプションについては、[Jestドキュメント](https://jestjs.io/docs/cli#reference)をご覧ください。
...
- [Testing Playground](https://testing-playground.com/) - 良いテストプラクティスを使用して要素をマッチングします。
