...
> **Good to know:** `async` server componentはReactエコシステムにおいて新しいため、Jestは現在それをサポートしていません。同期的なserver componentとclient componentの**ユニットテスト**は実行できますが、`async` componentには**E2Eテスト**を使用することをお勧めします。
...
> **Good to know**: 環境変数を直接テストするには、別のセットアップスクリプトまたは`jest.config.ts`ファイルで手動でロードします。詳細については、[テスト環境変数](/docs/app/guides/environment-variables#test-environment-variables)を参照してください。
