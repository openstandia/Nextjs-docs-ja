...
`next/legacy/image`から新しい`next/image`に危険を伴って移行し、インラインスタイルを追加し、未使用のpropsを削除します。
...
トップレベルページで自動的に注入される`url`プロパティを、`withRouter`とそれが注入する`router`プロパティを使用するように変換します。詳細はこちら：[https://nextjs.org/docs/messages/url-deprecated](https://nextjs.org/docs/messages/url-deprecated)
