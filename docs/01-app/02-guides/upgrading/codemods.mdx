...
Codemodsは、コードベースに対してプログラム的に実行される変換です。これにより、すべてのファイルを手動で確認することなく、多数の変更をプログラム的に適用することができます。
...
ページ/ルートエントリ（`page.js`、`layout.js`、`route.js`、`default.js`）や`generateMetadata` / `generateViewport` APIで`params`または`searchParams`のpropsへのプロパティアクセスを検出した場合、同期から非同期関数への変換を試み、プロパティアクセスをawaitします。非同期にできない場合（Client Componentなど）、`React.use`を使用してプロミスをアンラップします。
...
> **Good to know:** このcodemodが手動介入が必要な箇所を特定したが、正確な修正を決定できない場合、コードにコメントまたは型キャストを追加して、手動で更新する必要があることをユーザーに通知します。これらのコメントは**@next/codemod**で始まり、型キャストは`UnsafeUnwrapped`で始まります。これらのコメントが明示的に削除されるまで、ビルドはエラーになります。[詳細はこちら](https://nextjs.org/docs/messages/sync-dynamic-apis)。
...
`next/legacy/image`から新しい`next/image`への危険な移行を行い、インラインスタイルを追加し、未使用のpropsを削除します。
...
これは一例です。変換されるすべてのケース（およびテスト済みのケース）は、[`__testfixtures__`ディレクトリ](https://github.com/vercel/next.js/tree/canary/packages/next-codemod/transforms/__testfixtures__/url-to-withrouter)で確認できます。
