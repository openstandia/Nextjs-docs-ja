...
インラインスタイルを追加し、未使用のpropsを削除することで、`next/legacy/image`から新しい`next/image`に危険を伴って移行します。
...
トップレベルページで自動的に注入される廃止された`url`プロパティを`withRouter`とそれが注入する`router`プロパティを使用するように変換します。詳細はこちらをお読みください：[https://nextjs.org/docs/messages/url-deprecated](https://nextjs.org/docs/messages/url-deprecated)
...
これは一例です。変換され（およびテストされた）すべてのケースは、[`__testfixtures__`ディレクトリ](https://github.com/vercel/next.js/tree/canary/packages/next-codemod/transforms/__testfixtures__/url-to-withrouter)で見つけることができます。
