...
Codemodsは、コードベースに対してプログラム的に実行される変換です。これにより、すべてのファイルを手動で確認することなく、大量の変更をプログラム的に適用することができます。
...
> **Good to know:** このcodemodが手動介入が必要な箇所を特定した場合、正確な修正を判断できない場合でも、コードにコメントまたは型キャストを追加して、手動で更新する必要があることをユーザーに通知します。これらのコメントは**@next/codemod**で始まり、型キャストは`UnsafeUnwrapped`で始まります。
...
`next/legacy/image`から新しい`next/image`への移行を危険に行い、インラインスタイルを追加し、未使用のpropsを削除します。
...
トップレベルページで自動的に注入される`url`プロパティを`withRouter`とそれが注入する`router`プロパティを使用するように変換します。詳細はこちらをご覧ください：[https://nextjs.org/docs/messages/url-deprecated](https://nextjs.org/docs/messages/url-deprecated)
...
これは一例です。変換され（およびテストされ）たすべてのケースは、[`__testfixtures__`ディレクトリ](https://github.com/vercel/next.js/tree/canary/packages/next-codemod/transforms/__testfixtures__/url-to-withrouter)で見つけることができます。
