...
> **Good to know:**
>
> - ピア依存関係の警告が表示された場合、`react`と`react-dom`を提案されたバージョンに更新するか、警告を無視するために`--force`または`--legacy-peer-deps`フラグを使用する必要があるかもしれません。Next.js 15とReact 19が安定版になれば、これは不要になります。
...
移行の負担を軽減するために、プロセスを自動化する[codemodが利用可能](/docs/app/guides/upgrading/codemods#150)であり、APIは一時的に同期的にアクセスできます。
...
レイアウトやページ内のすべての`fetch`リクエストをキャッシュに入れるには、`export const fetchCache = 'default-cache'` [segment configオプション](/docs/app/api-reference/file-conventions/route-segment-config)を使用できます。個々の`fetch`リクエストが`cache`オプションを指定している場合は、それが優先されます。
...
`<Link>`や`useRouter`を介してページ間を移動する際、[ページ](/docs/app/api-reference/file-conventions/page)セグメントはクライアントサイドルーターキャッシュから再利用されなくなりました。ただし、ブラウザの戻る・進むナビゲーション中や共有レイアウトでは引き続き再利用されます。
...
[レイアウト](/docs/app/api-reference/file-conventions/layout)や[ローディング状態](/docs/app/api-reference/file-conventions/loading)は、ナビゲーション時に引き続きキャッシュされ再利用されます。
