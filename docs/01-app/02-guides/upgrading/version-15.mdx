...
> **Good to know:**
>
> - ピア依存関係の警告が表示された場合、`react`と`react-dom`を提案されたバージョンに更新するか、警告を無視するために`--force`または`--legacy-peer-deps`フラグを使用する必要があるかもしれません。Next.js 15とReact 19の両方が安定版になれば、これは不要になります。
...
移行の負担を軽減するために、プロセスを自動化する[コーデモッドが利用可能](/docs/app/guides/upgrading/codemods#150)であり、APIは一時的に同期的にアクセスできます。
...
`runtime` [segment config](/docs/app/api-reference/file-conventions/route-segment-config#runtime)は、以前は`edge`に加えて`experimental-edge`の値をサポートしていました。両方の設定は同じものを指しており、オプションを簡素化するために、`experimental-edge`が使用されるとエラーが発生するようになります。これを修正するには、`runtime`設定を`edge`に更新してください。[コーデモッド](/docs/app/guides/upgrading/codemods#app-dir-runtime-config-experimental-edge)が自動的にこれを行うために利用可能です。
...
レイアウトやページ内のすべての`fetch`リクエストをキャッシュにオプトインするには、`export const fetchCache = 'default-cache'` [segment configオプション](/docs/app/api-reference/file-conventions/route-segment-config)を使用できます。個々の`fetch`リクエストが`cache`オプションを指定している場合は、それが使用されます。

```js title="app/layout.js"
// これはrootレイアウトなので、アプリ内のすべてのfetchリクエストは
// 自分のキャッシュオプションを設定しない限りキャッシュされます。
...
[ルートハンドラー](/docs/app/api-reference/file-conventions/route)内の`GET`関数は、デフォルトではキャッシュされなくなりました。`GET`メソッドをキャッシュにオプトインするには、`export const dynamic = 'force-static'`のような[ルート設定オプション](/docs/app/api-reference/file-conventions/route-segment-config)をルートハンドラーファイルに使用できます。
...
`<Link>`や`useRouter`を介してページ間を移動する際、[ページ](/docs/app/api-reference/file-conventions/page)セグメントはクライアントサイドのルーターキャッシュから再利用されなくなりました。ただし、ブラウザの戻る・進むナビゲーション中や共有レイアウトでは引き続き再利用されます。
...
[レイアウト](/docs/app/api-reference/file-conventions/layout)と[ローディング状態](/docs/app/api-reference/file-conventions/loading)は、ナビゲーション時に引き続きキャッシュされ再利用されます。

## `next/font` {#next-font}

`@next/font`パッケージは、組み込みの[`next/font`](/docs/app/api-reference/components/font)に置き換えられました。[コーデモッドが利用可能](/docs/app/guides/upgrading/codemods#built-in-next-font)で、安全かつ自動的にインポートをリネームできます。
...
`NextRequest`の`geo`と`ip`プロパティは、これらの値がホスティングプロバイダーによって提供されるため削除されました。[コーデモッド](/docs/app/guides/upgrading/codemods#150)がこの移行を自動化するために利用可能です。
