...
- `useFormStatus`には、`data`、`method`、`action`などの追加キーが含まれるようになりました。React 19を使用していない場合、利用可能なキーは`pending`のみです。[詳細はこちら](https://react.dev/reference/react-dom/hooks/useFormStatus)。
...
以前は`edge`に加えて`experimental-edge`の値をサポートしていた`runtime` [セグメント設定](/docs/app/api-reference/file-conventions/route-segment-config#runtime)は、両方の設定が同じものを指しているため、オプションを簡素化するために、`experimental-edge`が使用されるとエラーが発生するようになりました。これを修正するには、`runtime`設定を`edge`に更新してください。[codemod](/docs/app/guides/upgrading/codemods#app-dir-runtime-config-experimental-edge)が自動的にこれを行うために利用可能です。
...
レイアウトやページ内のすべての`fetch`リクエストをキャッシュにオプトインするには、`export const fetchCache = 'default-cache'` [セグメント設定オプション](/docs/app/api-reference/file-conventions/route-segment-config)を使用できます。個々の`fetch`リクエストが`cache`オプションを指定している場合、それが使用されます。
...
[ルートハンドラー](/docs/app/api-reference/file-conventions/route)内の`GET`関数はデフォルトでキャッシュされなくなりました。`GET`メソッドをキャッシュにオプトインするには、`export const dynamic = 'force-static'`のような[ルート設定オプション](/docs/app/api-reference/file-conventions/route-segment-config)をルートハンドラーファイルに使用できます。
...
[レイアウト](/docs/app/api-reference/file-conventions/layout)や[ローディング状態](/docs/app/api-reference/file-conventions/loading)は引き続きキャッシュされ、ナビゲーション時に再利用されます。

## `next/font` {#next-font}

`@next/font`パッケージは、組み込みの[`next/font`](/docs/app/api-reference/components/font)に置き換えられました。[codemodが利用可能](/docs/app/guides/upgrading/codemods#built-in-next-font)で、安全かつ自動的にインポートをリネームします。
