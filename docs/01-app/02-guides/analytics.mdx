...
[Web Vitals](https://web.dev/vitals/)は、ウェブページのユーザーエクスペリエンスを捉えることを目的とした有用な指標のセットです。以下のWeb Vitalsがすべて含まれています：
...
> **Good to know**: [Google Analytics](https://analytics.google.com/analytics/web/)を使用している場合、`id`値を使用することで、メトリクスの分布を手動で構築することができます（パーセンタイルを計算するなど）

> ```js
> useReportWebVitals((metric) => {
>   // Google Analyticsを次の例のように初期化した場合は`window.gtag`を使用
