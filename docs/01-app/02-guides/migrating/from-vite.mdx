...
このファイルは、`'use client'`ディレクティブによって定義された[Client Component](/docs/app/building-your-application/rendering/client-components)です。クライアントコンポーネントは、クライアントに送信される前にサーバーで[HTMLにプリレンダリング](/docs/app/building-your-application/rendering/client-components#how-are-client-components-rendered)されます。
...
Next.jsは、Viteとは少し異なる方法で静的画像のインポートを処理します。Viteでは、画像ファイルをインポートすると、その公開URLが文字列として返されます：
