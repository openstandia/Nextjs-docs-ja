...
初期の`fallback`データはNext.jsによって自動的に処理されるため、以前に`data`が`undefined`であるかどうかを確認するために必要だった条件付きロジックを削除できます。データが読み込まれているとき、最も近い`<Suspense>`境界がサスペンドされます。
...
## 既存プロジェクトをNext.jsに移行する {#migrating-existing-projects-to-next-js}
