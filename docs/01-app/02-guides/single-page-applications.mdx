...
単一のPromiseを[遅延させてpropとしてクライアントコンポーネントに渡す](/docs/app/getting-started/fetching-data#with-the-use-hook)こともできますが、一般的にはこのパターンはReactのコンテキストプロバイダーと組み合わせて使用されます。これにより、カスタムReactフックを使用してクライアントコンポーネントからのアクセスが容易になります。
...
Pages Routerを使用しているSPAを既にお持ちの場合は、[App Routerを段階的に採用する方法](/docs/app/guides/migrating/app-router-migration)を学ぶことができます。
