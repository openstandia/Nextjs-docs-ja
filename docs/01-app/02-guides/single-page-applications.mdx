...
[Create React App](/docs/app/guides/migrating/from-create-react-app)や[Vite](/docs/app/guides/migrating/from-vite)のような厳密なSPAから移行する場合、URL状態を更新するためにシャロールートを持つ既存のコードがあるかもしれません。これは、Next.jsのファイルシステムルーティングを使用せずにアプリケーション内のビュー間で手動で遷移するのに役立ちます。
