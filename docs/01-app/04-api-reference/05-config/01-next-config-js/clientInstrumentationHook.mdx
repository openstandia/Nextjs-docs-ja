...
有効にすると、Next.js はプロジェクトのrootにある `instrumentation-client.ts` ファイルを検索し、ハイドレーションの前に同期的に実行します。このファイルは初期のJavaScriptチャンクにバンドルされ、他のコードが実行される前に分析やモニタリングを実行することができます。このファイル内のコードはクライアントサイドでのみ実行され、サーバーサイドレンダリング（SSR）中には実行されません。
