...
`opengraph-image` と `twitter-image` ファイル規約を使用すると、ルートセグメントに対して Open Graph および Twitter の画像を設定できます。
...
root セグメントから `opengraph-image` または `twitter-image` が配置されているセグメントまでの [dynamic route parameters](/docs/app/building-your-application/routing/dynamic-routes) オブジェクトを含むオブジェクト。
...
`opengraph-image` と `twitter-image` は、Pages および Layouts と同じ [route segment configuration](/docs/app/api-reference/file-conventions/route-segment-config) オプションを使用できる特別な [Route Handlers](/docs/app/building-your-application/routing/route-handlers) です。
...
#### ローカルアセットを使用した Edge runtime {#using-edge-runtime-with-local-assets}
...
#### ローカルアセットを使用した Node.js runtime {#using-node-js-runtime-with-local-assets}
