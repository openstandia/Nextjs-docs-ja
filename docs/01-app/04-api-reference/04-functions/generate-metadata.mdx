...
>
> これらの値が存在する場合、それらは`metadataBase`の**デフォルト値**として使用されます。そうでない場合、`http://localhost:${process.env.PORT || 3000}`にフォールバックします。これにより、Open Graph画像がローカルビルドとVercelプレビューおよび本番デプロイメントの両方で機能します。デフォルトを上書きする場合、環境変数を使用してURLを計算することをお勧めします。これにより、ローカル開発、ステージング、および本番環境のURLを設定できます。
>
...
> **Good to know**:
>
> - Open Graph画像には、[ファイルベースのメタデータAPI](/docs/app/api-reference/file-conventions/metadata/opengraph-image#image-files-jpg-png-gif)を使用する方が便利かもしれません。設定エクスポートと実際のファイルを同期する必要がある代わりに、ファイルベースのAPIは自動的に正しいメタデータを生成します。
...
歴史的な興味を持つ記録、文書、またはその他の資料のコレクションを説明します（[ソース](https://www.w3.org/TR/2011/WD-html5-20110113/links.html#rel-archives)）。
...
> **Good to know**:
>
> - これらのメソッドは現在、Client Componentsでのみサポートされていますが、初期ページロード時には依然としてサーバーサイドレンダリングされています。
...
ページメタデータは主にボットやクローラーを対象としているため、Next.jsはJavaScriptを実行し、完全なページDOMを検査できるボット（例：`Googlebot`）にメタデータをストリーミングします。ただし、メタデータは**HTML限定**のボット（例：`Twitterbot`）のレンダリングを引き続きブロックします。これらはクロール中にJavaScriptを実行できないためです。

Next.jsは、ストリーミングメタデータを提供するか、ブロックメタデータにフォールバックするかを判断するために、受信リクエストのユーザーエージェントを自動的に検出します。

このリストをカスタマイズする必要がある場合は、`next.config.js`の`htmlLimitedBots`オプションを使用して手動で定義できます。Next.jsは、この正規表現に一致するユーザーエージェントがWebページを要求する際にブロックメタデータを受け取ることを保証します。
