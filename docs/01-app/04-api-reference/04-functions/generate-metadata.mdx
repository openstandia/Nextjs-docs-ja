...
description: 'Next.jsアプリケーションにメタデータを追加して、検索エンジン最適化（SEO）とWebの共有性を向上させる方法を学びます。'
related:
  title: 'Next Steps'
...
> - 初回ロード時には、`generateMetadata`が完全に解決されるまでストリーミングがブロックされます。これには`loading.js`からのコンテンツも含まれます。
...
- `props` - 現在のルートのパラメータを含むオブジェクト：
  ...
  - `params` - [dynamic route parameters](/docs/app/building-your-application/routing/dynamic-routes)オブジェクトをrootセグメントから`generateMetadata`が呼び出されるセグメントまで含むオブジェクト。例：
...
`title`属性は、ドキュメントのタイトルを設定するために使用されます。シンプルな[string](#string)またはオプションの[template object](#template-object)として定義できます。
...
> **Good to know**:
>
> - `title.template`は、**子**ルートセグメントに適用され、定義されているセグメントには適用されません。つまり：
>
...
> - `layout.js`
>
>   - `title`（文字列）と`title.default`は、子セグメントのデフォルトタイトルを定義します（独自の`title`を定義していない場合）。存在する場合、最も近い親セグメントから`title.template`を補完します。
...
>   - `title`（文字列）は、ルートのタイトルを定義します。存在する場合、最も近い親セグメントから`title.template`を補完します。
>   - `title.absolute`は、ルートのタイトルを定義します。親セグメントからの`title.template`を無視します。
>   - `title.template`は、ページがルートの終端セグメントであるため、`page.js`では効果がありません。
...
- 設定されていない場合、`metadataBase`は[デフォルト値](#default-value)で**自動的に設定されます**。
...
> **Good to know**:
>
> - `metadataBase`は、URLベースの`metadata`フィールドに対して、root `app/layout.js`で設定されることが一般的です。
...
> - URLベースの`metadata`フィールドで相対パスを使用し、`metadataBase`を設定しない場合、ビルドエラーが発生します。
...
>
> これらの値が存在する場合、それらは`metadataBase`の**デフォルト値**として使用されます。そうでない場合は、`http://localhost:${process.env.PORT || 3000}`にフォールバックします。これにより、Open Graph画像がローカルビルドとVercelプレビューおよび本番デプロイメントの両方で機能します。デフォルトを上書きする際には、環境変数を使用してURLを計算することをお勧めします。これにより、ローカル開発、ステージング、および本番環境のURLを設定できます。
>
...
> **Good to know**:
>
> - Open Graph画像には、[ファイルベースのメタデータAPI](/docs/app/api-reference/file-conventions/metadata/opengraph-image#image-files-jpg-png-gif)を使用する方が便利な場合があります。設定エクスポートと実際のファイルを同期する必要があるのではなく、ファイルベースのAPIが自動的に正しいメタデータを生成します。
...
記録、文書、またはその他の歴史的に興味深い資料のコレクションを記述します（[出典](https://www.w3.org/TR/2011/WD-html5-20110113/links.html#rel-archives)）。
...
`<link>`要素には、外部リソースが必要になる可能性があることをブラウザに示唆するために使用できる`rel`キーワードがいくつかあります。ブラウザはこの情報を使用して、キーワードに応じたプリロード最適化を適用します。
