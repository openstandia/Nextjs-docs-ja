...
`notFound()` 関数を呼び出すと、`NEXT_NOT_FOUND` エラーがスローされ、それがスローされたルートセグメントのレンダリングが終了します。[**not-found** ファイル](/docs/app/api-reference/file-conventions/not-found) を指定することで、セグメント内でNot Found UIをレンダリングして、このようなエラーを優雅に処理することができます。
